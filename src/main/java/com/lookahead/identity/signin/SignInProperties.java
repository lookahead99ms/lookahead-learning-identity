...
public record SignInProperties(@DefaultValue("30m") Duration idleLifetime,
        ...
        @DefaultValue("2") int maximumActiveSessions) {
    public SignInProperties {
        ...
        if (maximumActiveSessions != 0 && maximumActiveSessions != 2)
            throw new IllegalArgumentException("Maximum active sessions must be zero (unlimited Local) or two");
    }
    ...
    public static SignInProperties defaults() {
        return new SignInProperties(Duration.ofMinutes(30),Duration.ofDays(7),Duration.ofMinutes(5),2);
    }
    public boolean unlimited() { return maximumActiveSessions == 0; }
    public Integer reportedLimit() { return unlimited() ? null : maximumActiveSessions; }
}
